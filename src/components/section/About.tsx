...
export default function About() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const update = () => setInfoHeight(el.offsetHeight);
    update();
  ...
  return (
    <section
      id="about"
      data-theme="light"
      className="relative w-full py-24 bg-white border-b-4 border-black text-black z-0"
    >
      <div className="scroll-float no-cursor-track w-full bg-black text-white py-6 border-y-4 border-black overflow-hidden mb-20 rotate-1 shadow-[8px_8px_0px_0px_#000] relative z-20">
      ...
      <div className="container mx-auto px-4 md:px-8 xl:px-16 max-w-[1600px] relative z-20 gsap-stagger-container">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-6 md:gap-8 items-stretch">
          <div className="scroll-float glare-hover md:col-span-1 xl:col-span-3 neo-card !shadow-[8px_8px_0px_0px_#000] p-0 overflow-hidden group bg-white w-full max-w-full flex flex-col min-h-0 gsap-stagger-item">
          ...
          <div className="scroll-float glare-hover md:col-span-1 xl:col-span-9 bg-black border-4 border-black shadow-[8px_8px_0px_0px_#000] p-4 sm:p-6 lg:p-8 text-white flex flex-col justify-center transform hover:-translate-y-1 transition-transform w-full min-h-0 gsap-stagger-item">
            ...
            <div className="space-y-3 sm:space-y-4 font-mono w-full text-base sm:text-lg md:text-xl lg:text-2xl">
              <p className="flex justify-between items-baseline gap-2 sm:gap-4 border-b border-white/20 pb-1 scroll-float min-w-0">
                ...
                <span className="text-right min-w-0 break-words">{fullName}</span>
              </p>
              <p className="flex justify-between items-baseline gap-2 sm:gap-4 border-b border-white/20 pb-1 scroll-float min-w-0">
                <span className="font-bold shrink-0 text-left">{t("about.edu")}:</span>
                <span className="text-right min-w-0 break-words">{education}</span>
              </p>
              <p className="flex justify-between items-baseline gap-2 sm:gap-4 border-b border-white/20 pb-1 scroll-float min-w-0">
                <span className="font-bold shrink-0 text-left">{t("about.loc")}:</span>
                <span className="text-right min-w-0 break-words">{location}</span>
              </p>
              <p className="flex justify-between items-baseline gap-2 sm:gap-4 border-b border-white/20 pb-1 scroll-float min-w-0">
                <span className="font-bold shrink-0 text-left">{t("about.lang")}:</span>
                <span className="text-right min-w-0 break-words">{languages}</span>
              </p>
            </div>
          </div>

          <div className="scroll-float glare-hover md:col-span-2 xl:col-span-5 neo-card !shadow-[8px_8px_0px_0px_#000] p-6 lg:p-8 flex flex-col justify-center gap-6 min-h-[300px] gsap-stagger-item">
          ...
          <div className="scroll-float glare-hover md:col-span-1 xl:col-span-4 bg-white border-4 border-black shadow-[8px_8px_0px_0px_#000] p-6 lg:p-8 flex flex-col justify-center items-center text-center hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all cursor-pointer min-h-[300px] gsap-stagger-item">
            <div className="font-mono text-sm font-bold bg-black text-white px-2 mb-2">
              {t("about.aestheticMode")}
            </div>
          ...
          <div className="scroll-float glare-hover md:col-span-1 xl:col-span-3 bg-white border-4 border-black shadow-[8px_8px_0px_0px_#000] p-6 lg:p-8 flex items-center justify-center text-center relative overflow-hidden min-h-[300px] gsap-stagger-item">
            <div
          ...
          <div className="scroll-float glare-hover md:col-span-2 xl:col-span-12 neo-card !shadow-[8px_8px_0px_0px_#000] p-6 lg:p-8 bg-zinc-100 min-h-[300px] gsap-stagger-item">
